...
from tasks.vcf.data import Genotype, GenotypeData, filterTable
    ...
    for setName, idSet in genotypeIDs.items():
        ...
        for id in tqdm(idSet, unit="id", desc=f"Processing {setName} IDs"):
            ...
            for j, column in enumerate(columns):
                ...
                if (
                    (column in id or id in column)
                    and (len(column) > 3 and len(id) > 3)
                    and (
                        id not in clinicalSampleIDmap
                        or clinicalSampleIDmap[id] not in seenSampleIDs
                        or any(
                            [subID in clinicalSampleIDmap.values() for subID in subIDs]
                        )
                    )
                ):
                    if id in clinicalSampleIDmap:
                        seenSampleIDs.update(
                            clinicalSampleIDmap[id]
                        )  # prevent duplicate genotypes from clinical data
                    else:
                        for (
                            subID
                        ) in (
                            subIDs
                        ):  # handle case where only external subject ID is available
                    ...
                    if config["vcfLike"]["compoundSampleIdDelimiter"] in column:
                        ...
                        matched = any(
                            columnValue == id for columnValue in delimitedColumn
                        )
                    ...
                    if (
                        ...
                        matched = any(
                            idValue in delimitedColumn for idValue in delimitedID
                        )
                if matched:
                    processed_genotypes = []
                    for i, genotype in enumerate(values[:, j]):
                        alleles = (
                            genotype.replace("'", "").split("/")
...
def processGenotypes(filteredVCF, clinicalSampleIDmap, caseIDs, controlIDs, config):
    ...
    controlGenotypeDict, missingControlIDs, resolvedControlIDs = (
...
def load(config):
    ...
    externalSamples = [
        pd.read_csv(
            externalTable["path"],
            sep=("\t" if ".tsv" in externalTable["path"] else ","),
            index_col=externalTable["idColumn"],
        )
    ...
    referenceVCF = (
        ...
        if config["vcfLike"]["frequencyMatchReference"]
        else None
    )

    annotatedVCF = annotatedVCF.set_index(config["vcfLike"]["indexColumn"])
    referenceVCF = (
        referenceVCF.set_index(config["vcfLike"]["indexColumn"])
    ...
    return (
        ...
        (
            referenceVCF.loc[referenceVCF.index.dropna()]
            if referenceVCF is not None
            else None
        ),
    )
...
def prepareCaseControlSamples(
    caseGenotypes, controlGenotypes, sample_frequencies, doBalance=True
):
...
def prepareDatasets(
    ...
    Returns:
        dict: embedding
    """
    minorIDs, balancedMajorIDs, excessMajorIDs, sampleFrequencies = (
        prepareCaseControlSamples(
    ...
    for label in tqdm(allCrossValGenotypes.columns, desc="Matching IDs", unit="ID"):
        for subsetType in ["excess", "toSample"]:
            idSet = excessMajorIDs if subsetType == "excess" else trainIDs
    ...
    allGenotypes = pd.concat(
        [
            ...
            *[
                holdoutControlSet
                for holdoutControlSet in holdoutControlGenotypes.values()
            ],
        ],
        axis=1,
    )
    ...
    if verbose:
        ...
        if len(holdoutCaseGenotypes) > 0 or len(holdoutControlGenotypes) > 0:
            for setsType, holdoutSets in {
                "case": holdoutCaseGenotypes,
                "control": holdoutControlGenotypes,
            }.items():
    ...
    print(
        f"Dropped {preCleanedVariantCounts - len(allGenotypes.index)} variants with insufficient variance (stddev < 0.1) or missing values"
    )
    ...
    embedding = {
        ...
        "excessMajorSamples": (
            scaler.fit_transform(excessMajorSamples).transpose()
            if not excessMajorSamples.empty
            else np.array([])
        ),
        "excessMajorSetName": (
            "excess case"
            if all([id in caseIDs for id in excessIDs])
            else (
                "excess control"
                if all([id in controlIDs for id in controlIDs])
                else "mixed excess"
            )
        ),
    ...
    if len(holdoutCaseGenotypes) > 0 or len(holdoutControlGenotypes) > 0:
        ...
        for setName, sampleSet in holdoutCaseGenotypes.items():
            ...
            holdoutSamplesBySetName[setName] = scaler.fit_transform(
                sampleSet
            ).transpose()
            holdoutIndexBySetName[setName] = np.array(sampleSet.columns.tolist())
            holdoutLabelsBySetName[setName] = np.array(
                [1] * len(sampleSet.columns.tolist())
            )
            if setName in holdoutControlGenotypes:
                holdoutControlGenotypes[setName] = holdoutControlGenotypes[setName].loc[
                    allGenotypes.index
                ]
                ...
                holdoutSamplesBySetName[setName] = np.concatenate(
                    [
                        ...
                        scaler.fit_transform(
                            holdoutControlGenotypes[setName]
                        ).transpose(),
                    ]
                )
                ...
                holdoutLabelsBySetName[setName] = np.concatenate(
                    [
                        ...
                        np.array(
                            [0] * len(holdoutControlGenotypes[setName].columns.tolist())
                        ),
                    ]
                )
        ...
        for setName, sampleSet in holdoutControlGenotypes.items():
            ...
            holdoutIndexBySetName[setName] = np.array(sampleSet.columns.tolist())
            holdoutLabelsBySetName[setName] = np.array(
                [0] * len(sampleSet.columns.tolist())
            )
        try:
            embedding = {
                **embedding,
...
def integrateExternalSampleIDs(filteredExternalSamples, config, caseIDs, controlIDs):
    ...
    """
    holdoutCaseIDs = {
        holdoutSetName: np.array([])
        for holdoutSetName in config["externalTables"]["holdoutSetNames"]
    }
    holdoutControlIDs = {
        holdoutSetName: np.array([])
        for holdoutSetName in config["externalTables"]["holdoutSetNames"]
    ...
    for i, externalTableMetadata in enumerate(config["externalTables"]["metadata"]):
        ...
        if (
            externalTableMetadata["setType"]
            in config["externalTables"]["holdoutSetNames"]
        ):
            if label == config["clinicalTable"]["caseAlias"]:
                holdoutCaseIDs[externalTableMetadata["setType"]] = np.append(
                    holdoutCaseIDs[externalTableMetadata["setType"]],
                    [id for id in filteredExternalSamples[i].index.to_numpy()],
                )
            elif label == config["clinicalTable"]["controlAlias"]:
                holdoutControlIDs[externalTableMetadata["setType"]] = np.append(
                    holdoutControlIDs[externalTableMetadata["setType"]],
                    [id for id in filteredExternalSamples[i].index.to_numpy()],
                )
...
def processInputFiles(config):
    ...
    clinicalSampleIDmap = {
        id: filteredClinicalData.loc[id, config["clinicalTable"]["subjectIdColumn"]]
        for id in filteredClinicalData.index.tolist()
    }
    caseIDs = caseIDsMask[caseIDsMask].index.to_numpy()
    controlIDs = controlIDsMask[controlIDsMask].index.to_numpy()

    caseIDs, controlIDs, holdoutDictCaseIDs, holdoutDictControlIDs = (
        integrateExternalSampleIDs(filteredExternalSamples, config, caseIDs, controlIDs)
    )
    ...
    (
        ...
        resolvedCaseIDs,
        resolvedControlIDs,
    ) = processGenotypes(filteredVCF, clinicalSampleIDmap, caseIDs, controlIDs, config)

    if config["sampling"]["shuffleLabels"]:
        combinedIDs = list(resolvedCaseIDs["crossval"]) + list(
            resolvedControlIDs["crossval"]
        )
    ...
    if referenceVCF is not None:
        (
            ...
            missingFreqReferenceCaseIDs,
            missingFreqReferenceControlIDs,
            resolvedFreqReferenceCaseIDs,
            resolvedFreqReferenceControlIDs,
        ) = processGenotypes(
        ...
        resolvedCaseIDs = list(
            set(resolvedCaseIDs["crossval"]).intersection(
                set(resolvedFreqReferenceCaseIDs["crossval"])
            )
        )
        resolvedControlIDs = list(
            set(resolvedControlIDs["crossval"]).intersection(
                set(resolvedFreqReferenceControlIDs["crossval"])
            )
        )
    else:
        freqReferenceCaseGenotypeDict = {}
    ...
    for alias, (IDs, genotypeDict) in {
        ...
        "controlAlias": (
            missingControlIDs["crossval"],
            controlGenotypeDict["crossval"],
        ),
    ...
    resolvedIDs = np.hstack(
        [
            list(caseGenotypeDict["crossval"].keys()),
            list(controlGenotypeDict["crossval"].keys()),
        ]
    )
    allCrossValGenotypes = (
        createGenotypeDataframe(
            {**caseGenotypeDict["crossval"], **controlGenotypeDict["crossval"]},
            filteredVCF,
        )
        .dropna()
        .astype(int)
    ...
    if referenceVCF is not None:
        freqReferenceAllCrossValGenotypes = (
            createGenotypeDataframe(
                {
                    **freqReferenceCaseGenotypeDict["crossval"],
                    **freqReferenceControlGenotypeDict["crossval"],
                },
                filteredReferenceVCF,
            )
            .dropna()
            .astype(int)
        )
    ...
    if isinstance(allCrossValGenotypes.index, pd.MultiIndex):
        ...
        allCrossValGenotypes = allCrossValGenotypes[
            allCrossValGenotypes.index.notnull()
        ]
    ...
    if referenceVCF is not None:
        ...
        matched_allele_frequencies = findClosestFrequencyVariant(
            reference_allele_frequencies, allele_frequencies
        )
        frequencyFilteredGenotypes = allCrossValGenotypes.loc[
            matched_allele_frequencies
        ].sort_index()
        referenceFrequencyFilteredGenotypes = freqReferenceAllCrossValGenotypes.loc[
            reference_allele_frequencies
        ].sort_index()
        print(
            f"Matched {len(frequencyFilteredGenotypes)} alleles to reference VCF {config['vcfLike']['frequencyMatchReference']}"
        )
    else:
        frequencyFilteredGenotypes = allCrossValGenotypes.loc[allele_frequencies.index]
    ...
    caseGenotypesDataframe = createGenotypeDataframe(
        caseGenotypeDict["crossval"], filteredVCF
    ...
    holdoutCaseGenotypesDataframe = (
        createGenotypeDataframe(
            {
                id: genotype
                for holdoutSet in holdoutCaseGenotypeDict.values()
                for id, genotype in holdoutSet.items()
            },
            filteredVCF,
        ).loc[frequencyFilteredGenotypes.index]
    ...
    holdoutControlGenotypesDataframe = (
        createGenotypeDataframe(
            {
                id: genotype
                for holdoutSet in holdoutControlGenotypeDict.values()
                for id, genotype in holdoutSet.items()
            },
            filteredVCF,
        ).loc[frequencyFilteredGenotypes.index]
    ...
    if referenceVCF is not None:
        referenceCaseGenotypesDataframe = createGenotypeDataframe(
            freqReferenceCaseGenotypeDict["crossval"], filteredReferenceVCF
    ...
    caseGenotypes = Genotype(
        caseGenotypesDataframe, resolvedCaseIDs["crossval"], "Case"
    )
    ...
    holdoutCaseGenotypes = {
        setName: Genotype(
            holdoutCaseGenotypesDataframe[
                list(holdoutCaseGenotypeDict[setName].keys())
            ],
            resolvedHoldoutCaseIDs[setName],
            setName,
        )
    ...
    holdoutControlGenotypes = {
        setName: Genotype(
            holdoutControlGenotypesDataframe[
                list(holdoutControlGenotypeDict[setName].keys())
            ],
    ...
    if referenceVCF is not None:
        referenceCaseGenotypes = Genotype(
            referenceCaseGenotypesDataframe,
            resolvedFreqReferenceCaseIDs["crossval"],
            "Reference Case",
        )
        referenceControlGenotypes = Genotype(
            referenceControlGenotypesDataframe,
            resolvedFreqReferenceControlIDs["crossval"],
            "Reference Control",
        )
    ...
    for attr in ["case", "control", "holdout_case", "holdout_control"]:
        if "holdout" not in attr:
            ...
            for setName in holdoutData:
                ...
                currentGenotypeData.to_csv(
                    f"{runPath}/holdout/{setName}/embedding_{attr}__{setName}.csv"
                )
